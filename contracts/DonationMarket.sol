...
contract DonationMarket {
    ...
    constructor(Token tokenAddress, DonationMarketStorage storageAddress)
        public
    {
    ...
    modifier listingDonorOnly(uint256 listingId) {
        require(
            tx.origin ==
                donationMarketStorage.getListingDonorAddress(listingId),
            "You are not the donor of this listing!"
        );
        _;
    }
    ...
    modifier validListingOnly(uint256 listingId) {
        require(
            donationMarketStorage.checkListing(listingId),
            "Invalid Listing!"
        );
        _;
    }
    ...
    function autoRemoveExpiredRequest() internal whenDeprecated {
        ...
        for (uint8 i; i < activeListing.length; i++) {
            ...
            for (uint256 req; req < requestId.length; req++) {
                if (
                    requestId[req] != 0 &&
                    block.timestamp >
                    donationMarketStorage.getRequestExpiry(requestId[req])
                ) {
    ...
    function cancelRequest(uint256 requestId, uint256 listingId)
        public
        ...
        stoppedInEmergency
    {
        ...
        tokenContract.transferToken(
            owner,
            tx.origin,
            donationMarketStorage.getListingAmount(listingId)
        );
    ...
    function approve(uint256 requestId, uint256 listingId)
        public
        noReentrancy
        validListingOnly(listingId)
        listingDonorOnly(listingId)
    ...
    {
        require(
            donationMarketStorage.checkRequest(requestId),
            "request has been taken down"
        );
        //transfer tokens
        locked = true;
        uint256 amount = donationMarketStorage.getRequestAmt(requestId);
        require(
            donationMarketStorage.getListingAmount(listingId) >= amount,
            "Insufficient balance in listing to approve this transaction"
        );
        tokenContract.transferToken(
            donationMarketStorage.getListingDonorAddress(listingId),
            donationMarketStorage.getRequestRecipientAddress(requestId),
            amount
        );
        locked = false;
        emit transferred(
    ...
    function addRequest(
        ...
        uint256 deadline,
        uint256 requestId
    ) public validListingOnly(listingId) isActive {
        require(
            tx.origin !=
                donationMarketStorage.getListingDonorAddress(listingId),
            "You cannot request for your own listing, try unlisting instead!"
        );
        uint256 expirationTime = deadline;
        donationMarketStorage.addRequest(
            requestId,
            listingId,
            recipientId,
            amt,
    ...
    function getRecipientRequest(uint256 listingId)
        public
        view
        listingDonorOnly(listingId)
        validListingOnly(listingId)
    ...
    {
        uint256[] memory requests = donationMarketStorage.getListingRequests(
            listingId
        );
    ...
    function getCategory(uint256 listingId)
        public
        view
        validListingOnly(listingId)
    ...
    function getAllListings() public view returns (uint256[] memory) {
        uint256[] memory allActiveListing = new uint256[](
            donationMarketStorage.getActiveListingCount()
        );
    ...
    function getActiveRequest(uint256 listingId)
        public
        view
        validListingOnly(listingId)
        listingDonorOnly(listingId)
        returns (uint256[] memory)
    {
        uint256[] memory allRequestId = donationMarketStorage
